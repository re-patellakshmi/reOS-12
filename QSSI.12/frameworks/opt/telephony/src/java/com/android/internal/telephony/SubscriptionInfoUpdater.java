...
public class SubscriptionInfoUpdater extends Handler {
    ...
    protected void handleSimLoaded(int phoneId) {
       ...
       if (isAllIccIdQueryDone()) {
            ...
            logd("mOnCertificationMode=" + mOnCertificationMode + ", isVerizon=" + isVerizon + ", mccmnc=" + mccmnc
                    + ", hasSetNewSimNwType=" + hasSetNewSimNwType);
            if (!hasSetNewSimNwType && mOnCertificationMode && isVerizon) {
                hasSetNewSimNwType = true;
                for (int i = 0; i < PROJECT_SIM_NUM; i++) {
                    if (true || mInsertSimState[i] == SIM_NEW) {
                        int curNwMode = Settings.Global.getInt(mPhone[i].getContext().getContentResolver(),
                                Settings.Global.PREFERRED_NETWORK_MODE + mPhone[i].getSubId(), -1);
                        logd("[" + i + "]detect new sim : curNwMode = " + curNwMode);
			// NETWORK_MODE_LTE_GSM_WCDMA = 9
			// set to lte/gsm/wcdma for verizon
                        if (curNwMode != 9) {
                            logd("[" + i + "]set nw type to LTE/UMTS/GSM for new sim");
                            Settings.Global.putInt(
                                    mPhone[i].getContext().getContentResolver(),
                                    Global.PREFERRED_NETWORK_MODE + mPhone[i].getSubId(),
                                    9);
                            mPhone[i].setPreferredNetworkType(9, null);
