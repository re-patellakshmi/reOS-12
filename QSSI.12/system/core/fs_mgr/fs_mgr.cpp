...
static void resize_f2fs(const std::string& blk_device, const std::string& fs_type,const std::string& key_loc) {
     uint64_t device_sz;
     uint32_t device_ss;
     uint64_t device_sn;
     int status = 0;
     ...
     */
     ...
     if (fs_match(key_loc, KEY_IN_FOOTER))
     device_sz -= CRYPT_FOOTER_OFFSET;

     if (is_f2fs(fs_type)) {
        if (access(F2FS_RESIZE_BIN, X_OK)) {
             LINFO << "Not running " << F2FS_RESIZE_BIN << " on " << blk_device
             << " (executable not in system image)";
                ...
                LINFO << "Running " << F2FS_RESIZE_BIN << " on " << blk_device;
                 LINFO << "F2FS total size is " << device_sz << " " <<"device_ss" << " " << device_ss;
                 ...
                 LINFO << "Running " << f2fs_resize2fs_argv[0] << " " << f2fs_resize2fs_argv[1] << " " << f2fs_resize2fs_argv[2]<< " " << f2fs_resize2fs_argv[3];

                 ret = logwrap_fork_execvp(ARRAY_SIZE(f2fs_resize2fs_argv), f2fs_resize2fs_argv,
                                                     &status, true, LOG_KLOG, false, nullptr);

                     if (ret < 0) {
                        /* No need to check for error in fork, we can't really handle it now */
                         LERROR << "F2FS Failed trying to run " << F2FS_RESIZE_BIN;
                         return;
...
static bool userdata_need_resize(const std::string& blk_device, const FstabEntry& entry, const void *sb) {
     uint64_t device_sz;
     uint64_t device_bc;
     ...
     if ((ioctl(fd, BLKGETSIZE64, &device_sz)) == -1) {
        ...
        LINFO << "f2fs device_bc " << device_bc;

         if (is_f2fs(entry.fs_type)) {
             f2fs_sb = (struct f2fs_super_block *)sb;
             LINFO << "f2fs f2fs_sb " << (uint64_t)f2fs_sb->block_count;
...
static int prepare_fs_for_mount(const std::string& blk_device, const FstabEntry& entry,
    ...
    LINFO << "f2fs fs_match";
    LINFO << "f2fs mount_point " << entry.mount_point.c_str();
    if (!strcmp(entry.mount_point.c_str(), "/data")) {
        LINFO << "blk_device== " << blk_device << " " << "entry.blkdevice== " << entry.blk_device;
        //if (fs_match("/dev/block/mmcblk0p77", entry.blk_device)) {
        if ( 1 ) {
            LINFO << "f2fs fs_match 1";
            if (is_f2fs(entry.fs_type) && userdata_need_resize(blk_device, entry, (void *)&f2fs_sb)) {
                LINFO << "f2fs resize.f2fs";
                goto resize;
    ...
    resize:
        resize_f2fs(blk_device, entry.fs_type, entry.key_loc);
