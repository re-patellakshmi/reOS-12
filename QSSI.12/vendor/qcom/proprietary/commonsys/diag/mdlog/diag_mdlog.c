...
Copyright (c)2007-2012, 2014-2021 Qualcomm Technologies, Inc.
All Rights Reserved.
	...
	if (info) {
		...
		if (info->si_int & DIAG_STATUS_OPEN) {
			if (info->si_int & DIAG_MD_MSM_MASK) {
				...
				} else if (info->si_int & DIAG_CON_RESERVED) {
					DIAG_LOGE("diag: DIAG_STATUS_OPEN on DIAG_CON_RESERVED\n");
					if (qdss_mask & DIAG_CON_RESERVED)
						diag_notify_qdss_thread(DIAG_MSM_MASK, DIAG_CON_RESERVED);
				} else {
			...
			if (info->si_int & DIAG_MD_MSM_MASK) {
				if (info->si_int & DIAG_CON_MPSS) {
					...
					DIAG_LOGE("diag: DIAG_STATUS_CLOSED on DIAG_CON_RESERVED\n");
				} else {
						...
						uint8 low_wm_val, uint8 high_wm_val)
{
	...
	if (proc_mask & DIAG_CON_RESERVED) {
		peripheral = DIAG_RESERVED_PROC;
		ret = diag_configure_peripheral_buffering_tx_mode(peripheral, tx_mode,
								low_wm_val, high_wm_val);
		if (ret != 1) {
			DIAG_LOGE("diag_mdlog: Failed configuring the peripheral: %d tx_mode: %d, ret: %d\n",
