/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
Copyright (c) 2015-2021 by Qualcomm Technologies, Inc.
All Rights Reserved.
	...
	while (1) {
		...
		for (dev_idx = NUM_PROC-1; dev_idx >= 0; dev_idx--) {
			if (periph_info.peripheral_type & (1 << dev_idx)) {
				...
				if (periph_info.peripheral_mask & DIAG_CON_RESERVED) {
					periph_info.peripheral_mask = periph_info.peripheral_mask ^ DIAG_CON_RESERVED;
					if (qshrink_kill_thread)
						return 0;
				}
				if (periph_info.peripheral_mask & DIAG_CON_APSS) {
