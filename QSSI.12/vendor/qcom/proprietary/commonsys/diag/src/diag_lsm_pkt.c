...
Copyright (c) 2007-2011, 2013-2016, 2018, 2020-2021 Qualcomm Technologies, Inc.
All Rights Reserved.
...
void diagpkt_delay_commit (void *pkt)
{
	...
	if (pkt_len > 0 && -1 != diag_fd) {
		...
		if (is_dci_pkt) {
			...
			num_bytes = diag_lsm_comm_write(diag_fd, (const void*)pdiag_data, pkt_len + DIAG_REST_OF_DATA_POS);
		}
		if(num_bytes != 0) {
...
void diagpkt_commit (void *pkt)
{
	...
	if ((ret = diag_lsm_comm_write(diag_fd, (const void*) temp, DIAG_REST_OF_DATA_POS + rsp_length)) != 0) {
		DIAG_LOGE("Diag_LSM_Pkt: Write failed in %s, bytes written: %d, error: %d\n", __func__, ret, errno);
		gPkt_commit_fail++;
	}
	DiagSvc_Free(temp, PKT_SVC_ID);
