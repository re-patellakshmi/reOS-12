...
/* DIAG_QDSS_PROCESSOR_RESERVED is just a dummy value, need to define proper value */
#define DIAG_QDSS_PROCESSOR_RESERVED	0x1300
...
#define DIAG_STM_RESERVED	0x400
...
static int diag_set_diag_transport(int peripheral_type, int peripheral, uint8 stm_cmd)
{
	...
	*(int*)ptr = USER_SPACE_RAW_DATA_TYPE;
	...
	case DIAG_GPDSP1_PROC :
		...
		break;
	case DIAG_RESERVED_PROC :
		req->processor_mask = DIAG_STM_RESERVED;
		break;
	default:
...
static int diag_set_diag_qdss_tracer(int peripheral_type, int peripheral, uint8 state)
{
	...
	*(int*)ptr = USER_SPACE_RAW_DATA_TYPE;
	...
	case DIAG_GPDSP1_PROC :
		...
		break;
	case DIAG_RESERVED_PROC :
		req->subsys_cmd_code += DIAG_QDSS_PROCESSOR_RESERVED;
		break;
	default:
...
void* qdss_config_thread(void* param)
{
	...
	while (1) {
		if (qdss_periph_info.peripheral_type & DIAG_MSM_MASK) {
			...
			if (msm_peripheral_mask & DIAG_CON_RESERVED) {
				diag_send_cmds_to_peripheral_init(MSM, DIAG_RESERVED_PROC);
				msm_peripheral_mask = msm_peripheral_mask ^ DIAG_CON_RESERVED;
				if (qdss_kill_thread == 1)
					return 0;
			}
	...
	while (1) {
		for (dev_idx = 1; dev_idx < NUM_PROC; dev_idx++) {
			if ((qdss_periph_info.peripheral_type & (1 << dev_idx)) &&
				...
				if (peripheral_mask & DIAG_CON_RESERVED) {
					diag_send_cmds_to_peripheral_init(dev_idx, DIAG_RESERVED_PROC);
					peripheral_mask = peripheral_mask ^ DIAG_CON_RESERVED;
					if (qdss_kill_thread == 1)
						return 0;
				}
		...
		if (num_bytes_read <= 0) {
			...
			if (qdss_kill_thread == 1) {
				if ((num_bytes_read == EINVAL) &&
					...
					DIAG_LOGD("diag: %s: signalling reception of -EINVAL\n", __func__);
					pthread_cond_signal(&qdss_kill_cond);
					pthread_mutex_unlock(&qdss_kill_mutex);
					DIAG_LOGD("diag: %s, Exit read thread once QDSS exit success signalled\n",
					__func__);
					return 0;
				}
			}
...
static void write_to_qdss_file(void *buf, int len, int type)
{
	...
	if (qdss_diag_fd_md[type] < 0) {
		...
		(void)std_strlprintf(qdss_file_name_curr[type],
							FILE_NAME_LEN, "%s%s%s%s%s%s",
							output_dir[type], "/diag_qdss_log",
							qdss_peripheral_name, "_", timestamp_buf,
							(bin_or_qdss_cond) ? ".qdss" : ".bin");
		...
		if (bin_or_qdss_cond) {
			ret = write_qdss_header(qdss_diag_fd_dev, type);
			if (ret <= 0) {
				DIAG_LOGE("Failed to write header to QDSS file (%d)\n", ret);
				return;
			}
			qdss_count_written_bytes += ret;
		}
	}
...
static void write_to_qdss_file_mdm(void *buf, int len, int type) {
	...
	if (qdss_diag_fd_md[type] < 0) {
		...
		if (use_qmdl2_v2)
			wait_for_diagid_guid_map_completion(type);
		bin_or_qdss_cond_mdm = use_qmdl2_v2 && diagid_guid_status[type];
		get_time_string(timestamp_buf, sizeof(timestamp_buf));
		(void)std_strlprintf(qdss_file_name_curr[type],
							FILE_NAME_LEN, "%s%s%s%s%s%s",
							output_dir[type], "/diag_qdss_log",
							qdss_peripheral_name, "_", timestamp_buf,
							(bin_or_qdss_cond_mdm) ? ".qdss" : ".bin");
		qdss_diag_fd_md[type] = open(qdss_file_name_curr[type],
								  O_CREAT | O_RDWR,
								  0644);
		...
		if (bin_or_qdss_cond_mdm) {
			ret = write_qdss_header(qdss_diag_fd_dev_mdm, type);
			if (ret <= 0) {
				DIAG_LOGE("Failed to write header to QDSS file (%d)\n", ret);
				return;
			}
			qdss_count_written_bytes_mdm += ret;
		}
	}
...
void diag_kill_qdss_threads(void)
{
	 ...
	 ****************************************************/
	 if (p_type_mask & DIAG_MSM_MASK) {
		...
		if (qdss_mask & DIAG_CON_RESERVED) {
			diag_send_cmds_to_peripheral_kill(MSM, DIAG_RESERVED_PROC);
			qdss_mask ^= DIAG_CON_RESERVED;
		}
	...
	if (device_mask & local_device_mask) {
		...
		for (dev_idx = 1; dev_idx < NUM_PROC; dev_idx++) {
			if ((remote_mask & (1 << (dev_idx - 1))) &&
				...
				if (qdss_mask & DIAG_CON_RESERVED) {
					diag_send_cmds_to_peripheral_kill(dev_idx, DIAG_RESERVED_PROC);
					qdss_mask ^= DIAG_CON_RESERVED;
				}
				if (qdss_mask & DIAG_CON_APSS) {
