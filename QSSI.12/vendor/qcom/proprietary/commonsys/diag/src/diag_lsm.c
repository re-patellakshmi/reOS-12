...
static boolean fill_pkt_buffer(unsigned char *ptr)
{
	...
	int i, index = 0, z;
	uint32 count_received_bytes = 0, update_count = 0;
	int status = 0;
	char temp_xml_buf[GUID_LIST_END_XML_TAG_SIZE];
	int ret = 0;
...
int write_qdss_header(int fd, int proc)
{
	...
	diag_get_secure_diag_info(proc);

	/* Copy qmdl header */
	memcpy(&header, &qshrink4_data[proc], sizeof(header));
	legacy_header_len = 10 + (header.guid_list_entry_count * GUID_LEN);
...
void log_to_device(unsigned char *ptr, int logging_mode, int size, int type)
{
	...
	if (fd_dev != -1) {
		if(logging_mode == MEMORY_DEVICE_MODE ) {
			errno = 0;
			if (use_qmdl2_v2 && (!diagid_query_status[type] || !diagid_guid_status[type]))
				return;
			if ((use_qmdl2_v2 || hdlc_disabled[type]) && write_qshrink_header) {
				...
				if (use_qmdl2_v2 && (type >= 0)) {
					...
					if (qmdl2_v2_header_inited[type] &&
						...
						/* Query for key on every new file */
						diag_get_secure_diag_info(type);

						/*
						 * Write the header information for the QSHRINK4 in
						 * case for qmdl2 v2 file.
						 */
						len = qshrink4_data[type].header_length - diagid_entry_size;

						/* Update header to v3 if secure key info available */
						if (keys_stored(type)) {
							qshrink4_data[type].header_length += get_keys_header_size(type);
...
int diag_send_data(unsigned char buf[], int bytes)
{
	...
	bytes_written = diag_lsm_comm_write(diag_fd,(const void*)buf, bytes);
...
int diag_get_periph_id_from_buf_periph_id(uint8 id)
{
	...
	case BUF_DIAG_RESERVED_PROC:
		if (buffering_mode_periph_map_supported)
			return DIAG_RESERVED_PROC;
		else
			return BUF_DIAG_RESERVED_PROC;
	default:
		return -1;
	}
}
