...
public class ImsRegistrationController implements Handler.Callback {
    ...
    //This is used to indicate that UE is PS
    //only attached.
    private final int CODE_IS_PS_ONLY_ATTACHED = 3002;
    ...
    private void maybeNotifySrvDomainChange() {
        if (mRegistrationStatus != ImsRegistrationInfo.NOT_REGISTERED) {
            Log.i(this, "checkSrvDomainChange IMS not deregistered.");
            return;
        }
        int extraErrCode = mSrvDomain == ImsRegistrationUtils.PS_ONLY ?
                CODE_IS_PS_ONLY_ATTACHED : ImsReasonInfo.CODE_UNSPECIFIED;
