...
public class DataStatusNotificationService extends Service {
    ...
    private final int MOBILE_DATA_DISABLED = 0;
    private final int MOBILE_DATA_ENABLED = 1;
                ...
                if (subId != SubscriptionManager.INVALID_SUBSCRIPTION_ID) {
                    if (stateExtra == TelephonyManager.SIM_STATE_LOADED) {
                        ...
                        if (!mSubEnabledMap.containsKey(subId) || !mSubEnabledMap.get(subId)) {
                            ...
                            log("enableContentObserver subId = " + subId);
                            enableContentObserver(roamingUri);
                        }
                        triggerOnChange(roamingUri);
    ...
    private void handleOnMultiSimConfigChanged(int activeModemCount) {
        ...
        // unregister phone state listener if transitioning from DSDS to SS
        for (int i = prevModemCount - 1; i >= activeModemCount; --i) {
            unRegisterPhoneStateListener(i);
        }
        mPhoneStateListeners = Arrays.copyOf(mPhoneStateListeners, activeModemCount);
    ...
    private class DataSettingsObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange, Uri uri) {
            ...
            if (uri != null) {
                ...
                int slotId = 0;
                int subId = 0;
                if (authority.equals("settings")) { // authority for roaming
                    if (mPhoneCount > 1) {
                        /*
    ...
    private void triggerOnChange(Uri roamingUri) {
    ...
    private void enableContentObserver(Uri roamingUri) {
    ...
    private Apn getPreferredApn(int subId, String selection) {
        ...
        if (cursor != null && cursor.getCount() > 0) {
            ...
            preferredApn = getApnFound(cursor,
                    TelephonyRegistry.getApnTypesStringFromBitmask(ApnSetting.TYPE_DEFAULT));
        }
        if(cursor != null) {
            cursor.close();
        }
        ...
        if (mPhoneStateListeners[phoneId] == null) {
            mPhoneStateListeners[phoneId] = new PhoneStateListener() {
                ...
                public void onUserMobileDataStateChanged(boolean enabled) {
                    log("onUserMobileDataStateChanged: enabled=" + enabled);
                    int mobile_data_status = enabled ? 1 : 0;
                    if (!mIsQcRilHookReady) {
                        Log.e(TAG, "QcRilhook not read. Bail out");
                        return;
                    }
    ...
    private void updateDataStatus(int phoneId, int subId, int state) {
        ...
        if (!mCallActive) {
            TelephonyManager tm = mTm.createForSubscriptionId(subId);
            int mobileDataStatus = tm.isDataEnabled() ? MOBILE_DATA_ENABLED : MOBILE_DATA_DISABLED;
            ...
            if (mobileDataStatus == 0) {
                ...
                boolean isDataAllowedInVoiceCall = tm.isMobileDataPolicyEnabled(
