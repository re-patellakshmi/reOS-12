...
public class QtiSubscriptionController extends VendorSubscriptionController {
    ...
    protected void handleDataPreference(int nextActiveSubId) {
        ...
        if (phone.getHalVersion().less(RIL.RADIO_HAL_VERSION_1_5)) {
    ...
    private int getSlotIdForSubId(int subId) {
        ...
        if (subMgr == null) return SubscriptionManager.INVALID_SIM_SLOT_INDEX;

        if (subMgr.isActiveSubscriptionId(subId)) return getSlotIndex(subId);
    ...
    private int getSlotIdForDeactivatedSub(int subId) {
        ...
        TelephonyManager telephonyMgr = mContext
                .getSystemService(TelephonyManager.class);
        UiccSlotInfo[] slotsInfo = telephonyMgr.getUiccSlotsInfo();
        if (slotsInfo == null) return SubscriptionManager.INVALID_SIM_SLOT_INDEX;
