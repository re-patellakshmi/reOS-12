...
public class QtiSubscriptionInfoUpdater extends SubscriptionInfoUpdater {
    ...
    static QtiSubscriptionInfoUpdater init(Looper looper, Context context,
        ...
        synchronized (QtiSubscriptionInfoUpdater.class) {
            if (sInstance == null) {
                sInstance = new QtiSubscriptionInfoUpdater(looper, context, sc);
            } else {
    ...
    private QtiSubscriptionInfoUpdater(Looper looper, Context context,
        ...
        for (int index = 0; index < SUPPORTED_MODEM_COUNT; index++) {
            ...
            if (phone != null) {
                phone.mCi.registerForIccRefresh(this,
                        EVENT_SIM_REFRESH, new Integer(index));
            }
        }
    }
    ...
    void addSubInfoRecord(int slotId, String iccId) {
        ...
        if (QtiPhoneUtils.getInstance().getCurrentUiccCardProvisioningStatus(slotId)
                != QtiUiccCardProvisioner.UiccProvisionStatus.PROVISIONED) {
            Rlog.w(LOG_TAG, "sim not provisioned, ignore[" + slotId + "] = " + iccId);
            return;
        }
    ...
    protected boolean areUiccAppsDisabledOnCard(int phoneId) {
        TelephonyManager telMgr = sContext.getSystemService(TelephonyManager.class);
    ...
    synchronized protected void updateSubscriptionInfoByIccId(int slotIndex,
        ...
        if (mIsRecordUpdateRequired[slotIndex] == true) {
            ...
            if (PhoneFactory.getPhone(slotIndex).getHalVersion().less(RIL.RADIO_HAL_VERSION_1_5)) {
