...
public class SubsidyLockUtils {
    ...
    public static void setPrimaryCardOnSlot(Context context, int slotId) {
        ...
        try {
            ...
            int[] subId = subscriptionManager.getSubscriptionIds(slotId);
            ...
            if (SubscriptionManager.getDefaultDataSubscriptionId() != subId[0]) {
                Log.d(TAG, "setPrimaryCardOnSlot: Setting the DDS to subId: " + subId[0]);
                subscriptionManager.setDefaultDataSubId(subId[0]);
            }

            if (getUserPreferredDataSubIdFromDB(context) !=  subId[0]) {
                Log.d(TAG, "Setting the user preferred data subId to " + subId[0]);
                setUserPreferredDataSubIdInDB(context, subId[0]);
            }
