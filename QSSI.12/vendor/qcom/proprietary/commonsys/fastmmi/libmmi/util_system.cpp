...
int GpioOutput(int gpionum, int value)
{
    int     fd;
    int     ret;
    char    buf[64];
    ...
    if (fd < 0) {
        MMI_ALOGE("[%s][%d]: open failed", __FUNCTION__, __LINE__);
        ret = -1;
        goto OUT2;
    }
    ...
    if (ret < 0) {
        MMI_ALOGE("[%s][%d]: write failed", __FUNCTION__, __LINE__);
        goto OUT;
    }
    close(fd);
...
int GpioReleaseTo(int gpionum, int value)
{
    int     fd;
    int     ret;
    char    buf[64];
    ...
    fd = open(buf, O_WRONLY);
    if (fd < 0) {
        MMI_ALOGW("[%s][%d]: gpio does not exist !", __FUNCTION__, __LINE__);
        ret = 0;
        goto OUT2;
    }
