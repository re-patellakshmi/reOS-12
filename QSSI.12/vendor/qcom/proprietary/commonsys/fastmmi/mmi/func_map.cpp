...
void switch_module(void *m) {
    ...
    layout *lay = NULL;
    MMI_ALOGI("[%s] for module:[%s];socketfd:[%d]",__FUNCTION__, mod->module, mod->socket_fd);
    ...
    if (is_camera_module(mod)) {
        button *btn_pass = lay->find_button_by_name(KEY_PASS);
        button *btn_fail = lay->find_button_by_name(KEY_FAIL);
        if (btn_pass)
            btn_pass->set_disabled(true);
        if (btn_fail)
            btn_fail->set_disabled(true);
    }
...
static void do_page_up(void *m) {
    ...
    if(is_factory_tab){
    	if(reconfig("/mnt/vendor/persist/FTM_AP/mmi.cfg") <= 0){
...
void do_show_fail(void *m) {
    ...
    if(curlay != NULL && curlay->m_listview != NULL) {
        ...
        btn = curlay->find_button_by_name(KEY_MAIN_ALL);
        btn_wait = curlay->find_button_by_name(KEY_MAIN_WAIT);
        btn_pass = curlay->find_button_by_name(KEY_MAIN_PASS);
        if(btn != NULL)
            btn->set_color(0x000000ff);
            ...
            if(btn_run_all != NULL) {
                if(btn_run_all->get_disabled()) {
                   ...
                   if(btn_failed != NULL){
                       ...
                       }//for check failed item
                       pass_num_result = lay->m_listview->get_pass_count();
                       fail_num_result = lay->m_listview->get_fail_count();
                       if ((pass_num_result == 0 )&& (fail_num_result == 0)){
                           btn_failed->set_color(0x987654ff);
                           btn_failed->set_text("TEST RESULT");
                           if(btn_wait != NULL){
                               btn_wait->set_color(0x000000ff);
                               btn_wait->set_text("NOT TEST");
                           }
                           if(btn_pass != NULL){
                               btn_pass->set_color(0x000000ff);
                               btn_pass->set_text("");
                           }
                       } else if(test_result) {
                       ...
                       } else if ((fail_num_result == 0)&&(pass_num_result != 0 )){
                           btn_failed->set_color(0x00ff00ff);
                           btn_failed->set_text("TEST RESULT");
                           if(btn_pass != NULL){
                               btn_pass->set_color(0x000000ff);
                               btn_pass->set_text("TEST PASS");
                           }
                           if(btn_wait != NULL){
                               btn_wait->set_color(0x000000ff);
                               btn_wait->set_text("");
...
static void do_show_all(void *m) {
	...
	if(curlay->m_listview != NULL) {
		...
		btn = curlay->find_button_by_name(KEY_MAIN_FAILED);
		MMI_ALOGI("set test result item");
		if(btn != NULL) {
			btn->set_color(0x000000ff);
			...
			if(btn_pass!=NULL){
				...
				MMI_ALOGI("btn sub pass NULL");
			}
			if(btn_wait!=NULL){
				btn_wait->set_text("");
			}else{
				MMI_ALOGI("btn wait test NULL");
			}
		}
	} else {
		MMI_ALOGE("Not find listview in layout");
...
static void *process_runall(void *m) {
    ...
    if(btn->get_smart_disabled()) {
        ...
        if(uart_fd < 0) {
            ...
            char *p = res; 
            char *result_pc = NULL;
            MMI_ALOGI("[%s] QFCT Result11", __FUNCTION__);
            snprintf(res, strlen("QFCTResult:"), "QFCTResult:");//send result to pc test,start begin with 'QFCTResult:'
            ...
            if(lay != NULL && lay->m_listview != NULL){
                ...
                for(iter = items->begin(); iter != items->end(); iter++){
                    item_t *obj = (item_t *) (*iter);
                    if(obj != NULL && obj->mod != NULL){
                        if(!strcmp(obj->mod->module,"SIMCARD1") ||
                        !strcmp(obj->mod->module,"SIMCARD2") ||
                        !strcmp(obj->mod->module,"WIFI") ||
                        !strcmp(obj->mod->module,"BLUETOOTH") ||
                        !strcmp(obj->mod->module,"GPS") ||
                        !strcmp(obj->mod->module,"SDCARD") ||
                        !strcmp(obj->mod->module,"EMMC") ||
                        !strcmp(obj->mod->module,"VIBRATOR")){
    ...
    MMI_ALOGI("[%s][%d] set smart fct false", __FUNCTION__, __LINE__);

    if(btn != NULL)
        btn->set_disabled(false);
    flush_result();
    do_show_fail(m);
    MMI_ALOGI("%s,test end.", __FUNCTION__);
    return NULL;
}
