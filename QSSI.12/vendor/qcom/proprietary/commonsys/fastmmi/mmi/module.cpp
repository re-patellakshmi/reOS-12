...
bool is_usb_module(module_info * mod) {
    return mod != NULL && !mod->config_list[KEY_LIB_NAME].empty()
        && !strcmp(MODULE_USB, mod->config_list[KEY_LIB_NAME].c_str());
}
...
void initial_screen(module_info * mod) {
    ...
    if(curlay != NULL) {
        ...
        if(tv != NULL) {
            if(!strcmp(mod->module, "SECOND LCD")) {
                main_fb();
            }else if(!strcmp(mod->module, "HANDSET")) {
                tv->set_text("PLAY MUSIC\nPLAY MUSIC\nPLAY MUSIC\nPLAY MUSIC\n");
            }else if(!strcmp(mod->module, "HEADSET LOOP")) {
                tv->set_text("HEADSET LOOP\nHEADSET LOOP\nHEADSET LOOP\nHEADSET LOOP\n");
            }else if(!strcmp(mod->module, "SPEAKER LOOP")) {
                tv->set_text("SPEAKER LOOP\nSPEAKER LOOP\nSPEAKER LOOP\nSPEAKER LOOP\n");
            }else if(!strcmp(mod->module, "NOISE MIC")) {
                tv->set_text("NOISE MIC\nNOISE MIC\nNOISE MIC\nNOISE MIC\n");
            }else if(!strcmp(mod->module, "CAMERA_AUX") || !strcmp(mod->module, "CAMERA_FRONT") || !strcmp(mod->module, "CAMERA_BACK")) {
                tv->set_text("Please Wait 10 Seconds\nPlease Wait 10 Seconds\n");
            }else {
...
static void send_cmd(module_info * mod, int cmd, const char *subcmd) {
    ...
    MMI_ALOGI("%s: %s: for module: [%s]: %d\n", __FILE__, __FUNCTION__, mod->module, cmd);
    if(mod->socket_fd < 0) {
...
static void send_run_mmi(module_info * mod) {
    MMI_ALOGI("send_run_mmi: [%s]: socket fd:[%d]\n",mod->module,mod->socket_fd);
...
int fork_launch_module(module_info * mod) {
    ...
    MMI_ALOGI("fork_launch_module: <%s> <%s> ", agent, para);
    ...
    if(pid == 0) {
        ...
        MMI_ALOGE("fork_launch_module: fork failed, error=%s\n", strerror(errno));
        return -1;
    } else if(pid > 0) {
        mod->pid = pid;
...
/**Fork module process*/
int fork_launch_module(int opt) {
    char para[1024] = { 0 };
    char agent[64] = { 0 };
#if 1
    if(opt == 0){
        if(check_file_exist(get_value(KEY_MMI_SERVER))) {
        strlcpy(agent, get_value(KEY_MMI_SERVER), sizeof(agent));
        }else{
            MMI_ALOGE("file not in position, error=%s\n", strerror(errno));
            return -1;
       }
    }else if(opt == 1){
        if(check_file_exist("/vendor/bin/mm_audio_ftm")) {
            strlcpy(agent, "/vendor/bin/mm_audio_ftm", sizeof(agent));
        }else{
            MMI_ALOGE("file not in position, error=%s\n", strerror(errno));
            return -1;
        }
    }
    MMI_ALOGI("fork '%s' for module:[SENSOR]", agent);

    int pid = fork();
    if(pid == 0){
        char *args[6] = { agent, NULL };
        int res = execv(agent, args);
        if(res < 0) {
            MMI_ALOGE("%s exec failed and exit, error=%s\n", agent, strerror(errno));
            exit(1);
        ...
        MMI_ALOGD("mmi_agent process pid=%d for module:[SENSOR]\n", pid);
    }
#endif
    return 0;
}
...
static void launch_module_bluetooth(module_info * mod) {
    ...
    if(fb_btn != NULL) {
        ...
        fb_btn->set_color(COLOR_BLUE);
		invalidate();
		MMI_ALOGI("module:[%s] waiting test result\n", mod->module);
		/*
...
static void *launch_module(void *module) {
    ...
    if (is_camera_module(mod)) {
        ...
        } while (mod->duration < 4);
        btn_fail->set_disabled(false);
        invalidate();
    }
