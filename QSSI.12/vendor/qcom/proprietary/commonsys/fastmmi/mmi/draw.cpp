...
static void draw_buttons(layout * lay, SkCanvas * skcan, draw_control_t * pdraw) {
    ...
    for(iter = lay->button_list.begin(); iter != lay->button_list.end(); iter++) {
        ...
        if(btn->get_visibility()) {
            ...
            if(!strcmp(btn->get_name(),"main_status")){
		            text_w = 730;
		            text_h = 52;            			
            }else if(!strcmp(btn->get_name(),"main_all")){
		            text_w = 77;
		            text_h = 52;  
            }else if(!strcmp(btn->get_name(),"main_failed")){
		            text_w = 220;
		            text_h = 52;  
            }else if(!strcmp(btn->get_name(),"main_runall")){
		            text_w = 187;
		            text_h = 52;  	
            }else if(!strcmp(btn->get_name(),"main_reset")){
		            text_w = 128;
		            text_h = 52;  
            }else if(!strcmp(btn->get_name(),"main_reboot")){
		            text_w = 180;
		            text_h = 52;  	
            }else if(!strcmp(btn->get_name(),"main_pageup")){
		            text_w = 62;
		            text_h = 52;  		
            }else if(!strcmp(btn->get_name(),"main_pagedown")){
		            text_w = 146;
		            text_h = 52;  	
            }
            else if(!strcmp(btn->get_name(),"display")){
		            text_w = 573;
		            text_h = 52;  
            }else if(!strcmp(btn->get_name(),"reboot_poweroff")){
		            text_w = 406;
		            text_h = 52;  	
            }else if(!strcmp(btn->get_name(),"instruction")){
		            text_w = 730;
		            text_h = 52;  	
            }else if(!strcmp(btn->get_name(),"reboot_ffbm")){
		            text_w = 450;
		            text_h = 52;  
            }else if(!strcmp(btn->get_name(),"reboot_android")){
		            text_w = 476;
		            text_h = 52;  
            }
            else if(!strcmp(btn->get_name(),"ok")){
		            text_w = 52;
		            text_h = 52;  
            }else if(!strcmp(btn->get_name(),"cancel")){
		            text_w = 171;
		            text_h = 52;  
            }
            else if(!strcmp(btn->get_name(),"title")){
		            text_w = 282;
		            text_h = 52;  
            }
            else if(!strcmp(btn->get_name(),"pass")){
		            text_w = 107;
		            text_h = 52;  
            }else if(!strcmp(btn->get_name(),"fail")){
		            text_w = 93;
		            text_h = 52;  
            }    
			...
			if(!strcmp(btn->get_name(),"wait_test")){
				rect_t *rect = btn->get_rect();
				pdraw->paint.setColor(SK_ColorYELLOW);
            text_w = 0;
            text_h = 133;

				align_center(rect, text_w, text_h*5, &point);
			skcan->drawSimpleText(btn->get_text(), strlen(btn->get_text()), SkTextEncoding::kUTF8, point.x + text_w / 14, point.y, font /*SkFont()*/, pdraw->paint);	

			} else if(!strcmp(btn->get_name(),"sub_pass")){  
				rect_t *rect = btn->get_rect();
				pdraw->paint.setColor(SK_ColorYELLOW);
            text_w = 0;
            text_h = 133;

				align_center(rect, text_w/4, text_h*5, &point);
			skcan->drawSimpleText(btn->get_text(), strlen(btn->get_text()), SkTextEncoding::kUTF8, point.x + text_w / 20, point.y, font /*SkFont()*/, pdraw->paint);	
        ...
        while(*p != '\0') {
            ...
            *ptr = '\0';
            ...
            if((last_text_y + text_h) < (rect->y + rect->h)) {
                ...
                skcan->drawSimpleText(tmp, strlen(tmp), SkTextEncoding::kUTF8, last_text_x, last_text_y, font, pdraw->paint);
            }
...
void draw_listviews(layout * lay, SkCanvas * skcan, draw_control_t * pdraw) {
    ...
    if(container->get_items() == NULL || container->get_item_num() <= 0)
        return;
    ...
    for(iter = container->get_items()->begin(); iter != container->get_items()->end(); iter++) {
		...
		srect.setXYWH(rect->x, rect->y, rect->x + 15, rect->h);
		if(obj->mod->result == FAILED && !res_tab) {
            pdraw->paint.setColor(0xFFFF0000);
			skcan->drawRect(srect, pdraw->paint);
        } else if(obj->mod->result == SUCCESS && !res_tab) {
            pdraw->paint.setColor(0xFF00FF00); 
			skcan->drawRect(srect, pdraw->paint);
        }
        /**Draw text*/
        ...
        skcan->drawSimpleText(text, strlen(text), SkTextEncoding::kUTF8, point.x, point.y, font, pdraw->paint);
    }
