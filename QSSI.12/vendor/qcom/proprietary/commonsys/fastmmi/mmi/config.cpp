...
int load_config(const char *path, unordered_map < string, module_info * >*modules, list < module_info * >*clients, int _sim_num, int _lcd) {
    char module[1024] = { 0, };
    char line[1024] = { 0, };
    char indicator = '=';
        ...
        if(module[0] != '\0') {
            ...
            if(*pname != '\0' && *pvalue != '\0') {
                ...
                if(!strcmp(pname,"display_name")){
                    ...
                    //(*label_modules)[get_string((string) pvalue)] = (string) module;
                }
                if(!strcmp(module,"SECOND LCD")){
                    if(_lcd == 1){
                        if(cur_mod != NULL){
                            if(!strcmp(pname,KEY_ENABLE)){
                                MMI_ALOGI("Parse CFG:FCT only one LCD.Disable the SECOND LCD\n");
    ...
    while(attr != NULL) {
        ...
        if(value != NULL) {
            if(!xmlStrcmp(attr->name, (const xmlChar *) "name")) {
                ...
                btn->set_color(strtoul(value, NULL, 0));
