...
static char mmi_auto_cfg_file[64];
...
static int parse_cfg_xml(const char *cfg_path,
                         ...
                         unordered_map < string, cfg_module_info * >*module_auto_map,
                         list < cfg_module_info * >*ordered_auto_module) {
    int ret = 0;
    char *value = NULL;
    xmlDocPtr doc = NULL;
    xmlNodePtr node = NULL;
    ...
    if(SUCCESS != parse_module_info(node->xmlChildrenNode, ordered_ui_module, module_ui_map) ||
       ...
       SUCCESS != parse_module_info(node->xmlChildrenNode, ordered_auto_module, module_auto_map)) {
    ...
    // parse default config for auto mode
    SET_NODE_NAME(key, XML_NODE_NAME_MODULES_AUTO);
    node = find_node(doc, &key);
    if(NULL == node) {
        ALOGE("Not found default config for pcba: [%s]", XML_NODE_NAME_MODULES_AUTO);
        xmlFreeDoc(doc);
        return FAILED;
    }
    ret = parse_module_info(node->xmlChildrenNode, ordered_auto_module, module_auto_map);
    if(SUCCESS != ret) {
        ALOGE("Parse default pcba config failed\n");
        xmlFreeDoc(doc);
        return FAILED;
    ...
    if(NULL != node) {
        ...
        // parse default config for auto mode of specified platform
        SET_NODE_NAME(key, XML_NODE_NAME_MODULES_AUTO);
        sub_node = find_node(node, &key);
        if(NULL != sub_node) {
            ret = update_module_info(sub_node->xmlChildrenNode, module_auto_map);
            if(SUCCESS != ret) {
                ALOGE("update default auto config failed for board: soc name=%s, subtype id=%s\n", board_info.soc_name,
                      board_info.subtype_id);
                xmlFreeDoc(doc);
                return FAILED;
            }
        } else {
            ALOGW("Not found config:%s in board(soc name=%s, subtype id=%s)\n", XML_NODE_NAME_MODULES_AUTO,
                  board_info.soc_name, board_info.subtype_id);
        }
        ...
        // parse enable modules for auto mode of specified platform
        SET_NODE_NAME(key, XML_NODE_NAME_ENABLE_MODULES);
        SET_ATTR_INFO(key.node_arrt1, XML_NODE_ATTR_MODE, XML_NODE_ATTR_VAL_AUTO);
        sub_node = find_node(node, &key);
        if(NULL != sub_node) {
            value = (char *) xmlNodeGetContent(sub_node);
            if(NULL == value) {
            ...
            reset_disable_flag(true, ordered_auto_module);
            set_disable_flag(doc, value, false, MODULE_SEPARATOR, module_auto_map);
            xmlFree(value);
        } else {
            ALOGW("Not found auto enable modules config in board(soc name=%s, subtype id=%s)\n", board_info.soc_name,
                  board_info.subtype_id);
        }
        ...
        if(NULL != sub_node) {
         ...
         // parse disable config for auto mode of specified platform
        SET_NODE_NAME(key, XML_NODE_NAME_DISABLE_MODULES);
        SET_ATTR_INFO(key.node_arrt1, XML_NODE_ATTR_MODE, XML_NODE_ATTR_VAL_AUTO);
        SET_ATTR_INFO(key.node_arrt2, XML_NODE_ATTR_PLATFORM_VER, board_info.platform_ver);
        if(NULL == (sub_node = find_node(node, &key))) {
        ...
        if(NULL != sub_node) {
            ...
            set_disable_flag(doc, value, true, MODULE_SEPARATOR, module_auto_map);
            xmlFree(value);
        } else {
            ALOGW("Not found pcba disable config in board(soc name=%s, subtype id=%s, platform ver=%s)\n",
                  board_info.soc_name, board_info.subtype_id, board_info.platform_ver);
...
char *get_mmi_cfg_file(int file_type) {
    ...
    } else if(CFG_TYPE_MMI_PCBA_CFG == file_type){
        cgf_file = mmi_pcba_cfg_file;
        snprintf(mmi_pcba_cfg_file, sizeof(mmi_pcba_cfg_file), "%s/%s", ftm_ap_dir, MMI_PCBA_CFG_FILE_NAME);
    } else {
        cgf_file = mmi_auto_cfg_file;
        snprintf(mmi_auto_cfg_file, sizeof(mmi_auto_cfg_file), "%s/%s", ftm_ap_dir, MMI_AUTO_CFG_FILE_NAME);
    }
...
static void release_memory(list < cfg_module_info * >*ordered_module_ui, list < cfg_module_info * >*ordered_module_pcba, list < cfg_module_info * >*ordered_module_auto){
    release_memory(ordered_module_ui);
    release_memory(ordered_module_pcba);
    release_memory(ordered_module_auto);
}
...
int create_mmi_cfg(void) {
    ...
    ordered_auto_module.clear();
    module_ui_map.clear();
    module_pcba_map.clear();
    module_auto_map.clear();

    // parse file: "/etc/mmi/mmi.xml"
    ret = parse_cfg_xml(MMI_XML_FILE, &module_ui_map, &ordered_ui_module, &module_pcba_map, &ordered_pcba_module, &module_auto_map, &ordered_auto_module);
    if(SUCCESS != ret) {
        ALOGE("file(%s) parse failed", MMI_XML_FILE);
        release_memory(&ordered_ui_module, &ordered_pcba_module, &ordered_auto_module);
        return FAILED;
    }
    ...
    ret = create_mmi_cfg(MMI_AUTO_CONFIG, &ordered_auto_module);
    if(SUCCESS != ret) {
        ALOGE("file(%s) create failed", MMI_AUTO_CONFIG);
        release_memory(&ordered_ui_module, &ordered_pcba_module, &ordered_auto_module);
        return FAILED;
    }
    release_memory(&ordered_ui_module, &ordered_pcba_module, &ordered_auto_module);
...
bool is_create_mmi_cfg(void) {
    ...
    if(!check_file_exist(MMI_CONFIG) || !check_file_exist(MMI_PCBA_CONFIG)|| !check_file_exist(MMI_AUTO_CONFIG)) {
