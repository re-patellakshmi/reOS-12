...
static int g_pid = -1;
unordered_map < string, string > stop_paras;
static void get_ftm_test_config(char *config, int size) {
    FILE *fp = NULL;
    char *soundCardInfo = NULL;
    char *sound_card = NULL;
    char *config_path = NULL;

    soundCardInfo = (char *)malloc(SIZE_256);
    sound_card = (char *)malloc(SIZE_256);
    config_path = (char *)malloc(SIZE_256);
    if (soundCardInfo == NULL || sound_card == NULL || config_path == NULL) {
        MMI_ALOGE("[%s] [%d] : malloc failed !", __FUNCTION__, __LINE__);
        goto OUT;
    }

    memset(soundCardInfo, 0, SIZE_256);
    memset(sound_card, 0, SIZE_256);
    memset(config_path, 0, SIZE_256);

    if((fp = fopen("/proc/asound/cards", "r")) != NULL) {
        while(fgets(soundCardInfo, SIZE_256, fp) != NULL)
            sscanf(soundCardInfo, "%*s%*s%*s%*s%s", sound_card);
        fclose(fp);

        snprintf(config_path, SIZE_256, "%s_%s", get_value("ftm_test_config"), sound_card);
    } else {
...
static int32_t module_run_mmi(const mmi_module_t * module, unordered_map < string, string > &params) {
    ...
    int ret = FAILED;
    char buf[32];
    char temp_agrs[SIZE_512];
    char *result = NULL;
    char *ftm_config_file = NULL;
    exec_cmd_t execmd;
    stop_paras = params;

    result = (char *)malloc(SIZE_2K);
    ftm_config_file = (char *)malloc(SIZE_256);
    if ((result == NULL) || (ftm_config_file == NULL)) {
        MMI_ALOGE("[%s] [%d] : malloc failed !", __FUNCTION__, __LINE__);
        ret = FAILED;
        goto OUT;
    }
    ...
    memset(result, 0, SIZE_2K);
    memset(ftm_config_file, 0, SIZE_256);

    if(!strcmp(params[KEY_MODULE_NAME].c_str(), "NOISE MIC")){
        snprintf(buf, sizeof(buf), "Waiting 3 Seconds......");
        module->cb_print(params[KEY_MODULE_NAME].c_str(), SUBCMD_MMI, buf, strlen(buf), PRINT_DATA);
    }

    get_ftm_test_config(ftm_config_file, SIZE_256);

    if(!strcmp(params["type"].c_str(), "autoloop")) {
        ...
        MMI_ALOGI("audio test autoloop mode\n");
        args[i++] = "/system/bin/audio_test";
        args[i++] = "/system_ext/etc/mmi/ftm_test";  
        ...
        memset(temp_agrs, 0, sizeof(temp_agrs));
        for(int j = 0; j < i && NULL != args[j]; j++)
            snprintf(temp_agrs + strlen(temp_agrs), sizeof(temp_agrs) - strlen(temp_agrs), "%s ", args[j]);
        MMI_ALOGI("exec command args:%s", temp_agrs);
    ...
    if(!strcmp(params["type"].c_str(), "loopback") || !strcmp(params["type"].c_str(), "play") || !strcmp(params["type"].c_str(), "mic")) {
        MMI_ALOGI("audio test loopback/play/mic mode\n");
    ...
    if(!strcmp(params["type"].c_str(), "mic")) {
        MMI_ALOGI("audio test play mode\n");
        const char *args_play[12] = { get_value("mm_audio_ftm"),
                                      "-c", ftm_config_file,
                                      "-tc", params["tc_play"].c_str(),
                                      "-d", params["duration"].c_str(),
                                      "-v", params["volume"].c_str(),
                                      "-file",params["file"].c_str(), NULL
        };
...
OUT:
    ...
    if (ftm_config_file != NULL) {
        free(ftm_config_file);
    }
...
static int32_t module_stop(const mmi_module_t * module) {
    ...
    if(!strcmp(stop_paras["type"].c_str(), "autoloop")) {
        args[i++] = "/system/bin/audio_test";
        args[i++] = "/system_ext/etc/mmi/ftm_test";
        args[i++] = stop_paras["tc"].c_str();
        args[i++] = "disable";
        args[i++] = stop_paras["duration"].c_str();

        memset(auto_agrs, 0, sizeof(auto_agrs));
        for(int j = 0; j < i && NULL != args[j]; j++)
            snprintf(auto_agrs + strlen(auto_agrs), sizeof(auto_agrs) - strlen(auto_agrs), "%s ", args[j]);
        MMI_ALOGI("exec command args:%s", auto_agrs);
        system(auto_agrs);
    }
