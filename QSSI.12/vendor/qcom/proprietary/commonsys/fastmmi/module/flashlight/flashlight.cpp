...
static bool thread_exit_status;
/**
...
static void light_on(bool enable) {
    if(enable) {
        ...
        if(check_file_exist(get_value("leds_switch_0"))) {
            MMI_ALOGE("%s: %s: leds_switch_0\n", __FILE__, __FUNCTION__);
            write_file(get_value("leds_switch_0"), "0");
        } else {
            MMI_ALOGE("%s: %s: leds_switch\n", __FILE__, __FUNCTION__);
...
static void *run_test(void *mod) {
    ...
    while(1) {
        if(thread_exit_status) return NULL;
        MMI_ALOGE("flash while loop\n");
        light_on(true);
    ...
    MMI_ALOGI("run test exit for module:[%s]", ((mmi_module_t *) mod)->name);
    return NULL;
}
