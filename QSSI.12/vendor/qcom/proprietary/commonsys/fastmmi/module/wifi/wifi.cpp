...
static int start_test(char *buf, uint32_t size) {
    ...
    if (!check_file_exist(get_value("wifi_ifconfig"))) {
        return FAILED;
    }
    ...
    int loop = 0;
    system("/vendor/bin/hw/wpa_supplicant -B -iwlan0 -Dnl80211 -c/system_ext/etc/mmi/wpa_supplicant_test.conf -O/data/misc/wifi/sockets -ddd");
    sleep(2);	
...
static int32_t module_run(const mmi_module_t * module, const char *cmd, unordered_map < string, string > &params) {
    ...
    MMI_ALOGI("module run finished for module:[%s], subcmd=%s, ret=%d\n", module->name, MMI_STR(cmd), ret);
   /** Default RUN mmi*/
    return ret;
}
