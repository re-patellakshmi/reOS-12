...
public class SMLParametersManager {
    ...
    public void setSMLUDKParameters() {
        Container udkConfig = mSMLConfig.getContainer(SML_UDK_PARAMS_TAG);
        if (udkConfig != null) {
            byte[] payload = fillSMLUDKParams(udkConfig);
    ...
    private byte[] fillSMLUDKParams(Container container) {
        byte[] smlUDKParams = new byte[SIZE_OF_SML_UDK_PARAMS];
        int startPos = 0;
