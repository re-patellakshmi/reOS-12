   ...
   public void clccResponse(int index, int direction, int call_status, int mode, boolean mpty,
    ...
    public boolean shouldCallAudioBeActive() {
        ...
        Log.d(TAG, "shouldCallAudioBeActive: " +
                   " callState: " + mPrevTelephonyState.mState +
                   " numHeld:" + mPrevTelephonyState.mNumHeld +
                   " numActive:" + mPrevTelephonyState.mNumActive);
        if (isInbandRingingEnabled() &&  mPrevTelephonyState.mState == CCHalConstants.CALL_STATE_INCOMING) {
            ret = true;
        } else if (mPrevTelephonyState.mNumActive > 0 || mPrevTelephonyState.mNumHeld > 0 ||
            (mPrevTelephonyState.mState != CCHalConstants.CALL_STATE_IDLE &&
            mPrevTelephonyState.mState != CCHalConstants.CALL_STATE_DISCONNECTED &&
            mPrevTelephonyState.mState != CCHalConstants.CALL_STATE_INCOMING)) {
            ret = true;
        }
    ...
    public  boolean  phoneStateChanged(int numActive, int numHeld, int callState, String number, int type,
      ...
      int profileID = mActiveDevMgrService.getActiveProfile(ApmConst.AudioFeatures.CALL_AUDIO);
      ...
      if (mPrevTelephonyState != null && mPrevTelephonyState.mNumActive == 1 &&
            ...
            currentTelephonyState.mState == CCHalConstants.CCS_STATE_DISCONNECTED) {
    ...
    private boolean isActiveDevice(BluetoothDevice device) {
        ...
        if (mActiveDevMgrService != null) {
            ret = Objects.equals(mGroupDevice, mActiveDevMgrService.getActiveDevice(ApmConst.AudioFeatures.CALL_AUDIO));
        }
    ...
    public boolean onCallControlPointChangedRequest(int op, int[] call_indices, int count, String dialNumber, BluetoothDevice device ) {
       ...
       switch(op) {
         ...
         case CCHalConstants.BTCC_OP_LOCAL_HLD:{
            ...
            int idx = CCHalConstants.BTCC_DEF_INDEX_FOR_FAILURES;
            Log.d(TAG, "callIndex: " + callIndex);
            if (isActiveDevice(device) && CallControl.holdCall(device, callIndex) == true) {
                res = CCHalConstants.BTCC_OP_SUCCESS;
                idx = callIndex;
            } else {
                res = CCHalConstants.BTCC_OP_NOT_POSSIBLE;
         ...
         case CCHalConstants.BTCC_OP_LOCAL_RETRIEVE: {
            ...
            int res;
            int idx = CCHalConstants.BTCC_DEF_INDEX_FOR_FAILURES;
            if (isActiveDevice(device) && CallControl.processChld(device, chld) == true) {
                res = CCHalConstants.BTCC_OP_SUCCESS;
                idx = call_indices[0];
            } else {
                res = CCHalConstants.BTCC_OP_NOT_POSSIBLE;
