...
def func_generateGoldenAbiDumpsforTechPackages(self):
    ...
    if abi_config_dict:
        ...
        for config in abi_config_dict:
            if config["abi_type"] == "techpackage" and os.path.exists(techpack_info_file):
...
def abi_checker_for_vndk_libs():
    ...
    if (str(Constants.platform_version).isdigit()):
      platform_version_suffix = Constants.platform_version
    else :
      try:
        platform_version_suffix = Constants.supported_platform_version[Constants.platform_version]
      except KeyError:
        Logger.logInternal.error("Platform version not supported : " + Constants.platform_version)
        return
