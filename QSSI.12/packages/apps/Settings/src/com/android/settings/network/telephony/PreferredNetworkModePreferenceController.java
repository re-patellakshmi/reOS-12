...
public class PreferredNetworkModePreferenceController extends TelephonyBasePreferenceController
    ...
    @VisibleForTesting
    Integer mCallState;
    private boolean isModemSup = true;
    private int vaialType;
    private boolean mOnCertificationMode = SystemProperties.getBoolean("persist.certification.mode", false);
    private boolean isVerizon = TelephonyManager.getDefault().isVerizon();
    ...
    public void displayPreference(PreferenceScreen screen) {
        ...
        if (mOnCertificationMode && isVerizon) {
            listPreference.setEntries(R.array.preferred_network_mode_verizon_choices);
            listPreference.setEntryValues(R.array.preferred_network_mode_verizon_values);
        }
    }
    ...
    public void updateState(Preference preference) {
        ...
        vaialType = Settings.Global.getInt(mContext.getContentResolver(),
                Settings.Global.PREFERRED_NETWORK_MODE + mSubId, RILConstants.PREFERRED_NETWORK_MODE);
        final ListPreference listPreference = (ListPreference) preference;
        listPreference.setValue(Integer.toString(vaialType));
        listPreference.setSummary(getPreferredNetworkModeSummaryResId(vaialType));
    ...
    public boolean onPreferenceChange(Preference preference, Object object) {
        ...
        isModemSup = mTelephonyManager.setPreferredNetworkType(mSubId, newPreferredNetworkMode);
        final ListPreference listPreference = (ListPreference) preference;
        if (isModemSup) {
            listPreference.setSummary(getPreferredNetworkModeSummaryResId(newPreferredNetworkMode));
            Settings.Global.putInt(mContext.getContentResolver(), Settings.Global.PREFERRED_NETWORK_MODE + mSubId,
    ...
    private int getPreferredNetworkModeSummaryResId(int NetworkMode) {
        switch (NetworkMode) {
            ...
            case TelephonyManagerConstants.NETWORK_MODE_LTE_GSM_WCDMA:
                if (mOnCertificationMode && isVerizon) {
                    return R.string.preferred_network_mode_lte_umts_gsm;
                }
