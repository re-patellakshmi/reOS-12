...
public class CellBroadcastAlertService extends Service {
    ...
    public boolean shouldDisplayMessage(SmsCbMessage message) {
        ...
        Log.d(TAG, "shouldDisplayMessage "+ range + messageLanguage);
        if (range != null && range.mFilterLanguage) {
            ...
            Log.d(TAG, "shouldDisplayMessage secondLanguageCode: "+ secondLanguageCode);
            //If we configured to receive alerts in a second language and the current message's language is the same as the second language configuration.
            if (!secondLanguageCode.isEmpty() && message.getLanguageCode() != null && secondLanguageCode.equalsIgnoreCase(message.getLanguageCode())) {
