...
{
(	cd ./QSSI.12/
	source build/envsetup.sh && lunch qssi-wifionlyuserdebug && bash build.sh dist --qssi_only EXPERIMENTAL_USE_OPENJDK9=1.8 $1
)
}
...
{
(	cd ./UM.9.15/
	source build/envsetup.sh && lunch bengal-wifionlyuserdebug && bash build.sh dist --target_only EXPERIMENTAL_USE_OPENJDK9=1.8 $1
)
}
